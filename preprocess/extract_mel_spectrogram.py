...
def get_spectrogram(audio_path, save_dir, length):
    ...
    mel_spec = librosa.feature.melspectrogram(y=y, sr=22050, n_fft=1024, hop_length=256, n_mels=128, fmin=0, fmax=22050/2.)
    
    mel_spec = mel_spec[:, :860] # shape (n_mels, 862) -> (n_mels, 860). why???
...
if __name__ == '__main__':
    ...
    paser.add_argument("-l", "--length", type=int, default=220500)
    paser.add_argument("-n", '--num_worker', type=int, default=20)
    ...
    with Pool(args.num_worker) as p:
        for _ in tqdm(p.imap_unordered(partial(get_spectrogram, save_dir=output_dir, length=length), audio_paths), total=len(audio_paths)):
            pass
